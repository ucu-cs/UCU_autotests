...
int main() {
    ...
    read_file_uint32(input, SIZE, "../../test_arrays/13_prime_32uint/input_32uint.lst");
    read_file_uint8(expected, SIZE, "../../test_arrays/13_prime_32uint/prime.lst");

    func(input, is_prime, SIZE);

    for (size_t i = 0; i < SIZE; ++i) {
        if (is_prime[i] != expected[i]) {
            printf("ERROR: for input[%zu] = %u expected %u but got %u\n",
